...
**코드베이스 파일 수정 시 (`.md` 제외) 반드시 Vercel 빌드 성공을 확인해야 작업 완료로 간주함.**

작업 완료 조건:
1. 코드 변경사항 푸시
2. Vercel 빌드 상태 확인 (pending → success/failure)
3. **빌드 성공 시**: 작업 완료
4. **빌드 실패 시**: 에러 수정 → 다시 푸시 → 빌드 성공까지 반복

```
코드 수정 → 푸시 → Vercel 빌드 대기 → 성공 확인 → 작업 완료
                              ↓ 실패
                        에러 수정 → 다시 푸시 → (반복)
```

**예외**: `.md`, `.txt` 등 문서 파일만 수정한 경우는 빌드 확인 불필요
